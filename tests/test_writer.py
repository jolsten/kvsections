...
def test_writer_rejects_line_separators_other_than_newline(text):
    with pytest.raises(ValueError, match="control or non-ASCII"):
        CommentDocument([TextSection("COMMENT", text)]).dumps()
...
def test_writer_rejects_a_section_of_the_wrong_kind():
    ...
    smuggled.sections["COMMENT"] = Section("COMMENT", X="1")  # bypasses add()
    with pytest.raises(ValueError, match="must hold free text"):
        smuggled.dumps()
    with pytest.raises(ValueError, match="must hold key/value pairs"):
        Document([TextSection("NOTES", "hello world")]).dumps()

    class Doc(Document):
        notes = SectionField(TextSection, "NOTES")

    assert Doc([TextSection("NOTES", "hello world")]).dumps(width=None) == (
        "NOTES hello world\n"
    )
...
def test_any_input_ending_is_written_as_lf(newline):
    ...
    assert doc.dumps(width=None) == "A X=1\nB Y=2\n"
    assert doc.dumps(width=None, newline="\r\n") == "A X=1\r\nB Y=2\r\n"
...
def test_writer_rejects_objects_that_are_not_sections():
    doc = Document([Section("A", X="1")])
    doc.sections["B"] = "not a section"  # bypasses add()
    with pytest.raises(TypeError):
        doc.dumps()
    section = Section("A")
    section.fields["K"] = 5  # bypasses normalization
    with pytest.raises(TypeError):
        Document([section]).dumps()
    comment = TextSection("COMMENT")
    comment.text = 5
    with pytest.raises(TypeError):
        CommentDocument([comment]).dumps()
